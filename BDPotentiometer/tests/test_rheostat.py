...
class TestRheostat(unittest.TestCase):
    ...
    def test_set_value(self):
        rheo = DigitalRheostatDevice(max_value=128, default_value=64, channels=10, r_ab=10e3, r_w=75)
        self.assertEqual(rheo._set_value(ch=0, value=129), 128)
        self.assertEqual(rheo._set_value(ch=0, value=10), 10)
        self.assertEqual(rheo._set_value(ch=0, value=-129), 0)
        with self.assertRaises(ValueError):
            rheo._set_value(ch=10, value=100)
    ...
    def test_set_r(self):
        ...
        r_ab = 10e3
        r_w = 75
        rheo = DigitalRheostatDevice(max_value=max_value, default_value=64, channels=2, r_ab=r_ab, r_w=r_w)
