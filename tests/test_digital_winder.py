...
class TestDigitalWinder(unittest.TestCase):
    ...
    def test_rwa_rwb(self):
        ...
        for resistance in np.linspace(
            ...
            self.assertEqual(
                digital_winder.r_wa, digital_winder.potentiometer.r_wa(pos)
            )
            self.assertEqual(
                digital_winder.r_wb, digital_winder.potentiometer.r_wb(pos)
            )
    ...
    def test_spi_digital_winder(self):
        ...
        digital_winder = SpiDigitalWinder(
            spi=None, potentiometer=pot, max_value=128, parameters_locked=False
        )
